 ...
 * */
public class PriorityQueueBinaryHeap extends PriorityQueueTA<Integer> {
    ...
    private Integer minChild(int i){
        ...
        //If the two children are out of bound, return 0
        if(c1 >= size()){
            return 0;
        }
        //If the second child is out of bound, return the first one
        else if(c2 >= size()){
            return c1;
        }
        //If the children are not out of bound, return the smallest
        else if(H.get(c1) < H.get(c2)){
            return c1;
        }
        else{
            return c2;
